...
def print_help():
    ...
    print(  "p [x0] [xn] [y] [n] -- set the parameters for solving;\n\n"
            "calc -- calculate;\n\n"
...
def split_args(inp):
    ...
    print(  "x0 = " + frm(x0) + "; xn = " + frm(xn) + "; "
            "y0 = " + frm(y0) + "; n = "  + str(n)  + ". " )

    return x0, xn, y0, n


def plot_solution(x0, xn, solution, c):
...
def method(x0, xn, y0, n, func):
    y, x, step, xarr, yarr = y0, x0, (xn-x0)/n, [], []
    for i in range(n):
        x       += step
        y       += step * func(x, y)
        xarr    += [x]
...
def main_loop():
    ...
    while True:
        ...
        if re.match(r"^eq [0-2] -?\d+.?\d*$", inp):
            func, solution, c = ret_functions(inp)
        elif re.match(r"^p -?\d+.?\d* -?\d+.?\d* -?\d+.?\d* \d+$", inp):
            x0, xn, y0, n = split_args(inp)
